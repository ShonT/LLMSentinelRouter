    ...
    try:
        ...
        result = await router.route(
            ...
            messages=[
                {
                    "role": "user",
                    "content": "Explain the implications of quantum entanglement on information theory and computational complexity.",
                }
            ],
        )
...
async def main():
    ...
    print(
        f"  DeepSeek API Key: {'✅ Set' if settings.deepseek_api_key != 'dummy_key' else '❌ Not Set'}"
    )
    print(
        f"  Anthropic API Key: {'✅ Set' if settings.anthropic_api_key != 'dummy_key' else '❌ Not Set'}"
    )
    print()
