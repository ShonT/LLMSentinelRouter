...
def verify_dockerfile():
    ...
    if 'CMD ["gunicorn"' in content:
        return False, "❌ Dockerfile CMD still uses array form (won't interpolate WORKERS)"
...
def verify_specific_fixes():
    ...
    if router_file.exists():
        ...
        if 'self.audit = AuditLogger(' in content:
            issues.append("❌ router_logic.py still uses AuditLogger instead of LoggingAudit")
    ...
    if budget_file.exists():
        ...
        if 'with_for_update()' in content:
            issues.append("✅ budget.py uses with_for_update() to prevent race conditions")
        else:
    ...
    if test_file.exists():
        ...
        if 'return_value=(0.3, "weak")' in content or 'return_value=(0.8, "strong")' in content:
            issues.append("❌ test_router.py still uses 2-tuple mocks (should be 3-tuple)")
