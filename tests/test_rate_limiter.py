...
class TestRateLimitWindow:
    ...
    async def test_daily_reset(self):
        ...
        window.last_daily_reset = datetime.now(timezone.utc) - timedelta(
            days=1, hours=1
        )
...
class TestRateLimiter:
    ...
    async def test_record_and_check(self):
        ...
        allowed, reason, usage = await limiter.check_rate_limits(
            "model-a", rpm_limit=10, tpm_limit=1000
        )
    ...
    async def test_estimated_tokens_projection(self):
        ...
        allowed, reason, usage = await limiter.check_rate_limits(
            "model-z", tpm_limit=1000, estimated_tokens=100
        )
...
async def test_concurrent_access():
    ...
    await asyncio.gather(add_requests(), add_requests(), add_requests())
