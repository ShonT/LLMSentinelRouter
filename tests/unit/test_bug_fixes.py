...
class TestBudgetOCC:
    ...
    def test_check_and_reserve_budget_exceeds_limit(self, budget_manager, test_db):
        ...
        success, version = budget_manager.check_and_reserve_budget(
            "test_occ_3", 
            estimated_cost=2.0  # Would exceed 5.0 limit
        )
        
        assert success is False
        assert version is None
    
    def test_check_and_reserve_budget_concurrent_conflict(self, budget_manager, test_db):
    ...
    def test_confidence_based_on_routable_calls_only(self, cache):
        ...
        for i in range(2):
            cache.record_interaction(
                ...
                latency_ms=100.0, judge_invoked=True,
                judge_latency_ms=50.0, complexity_score=0.2,
                impact_scope="LOW", cost=0.001, total_tokens=50,
            )
        ...
        cache.record_interaction(
            ...
            latency_ms=500.0, judge_invoked=True,
            judge_latency_ms=50.0, complexity_score=0.8,
            impact_scope="HIGH", cost=0.05, total_tokens=500,
        )
        ...
        for i in range(5):
            cache.record_interaction(
                ...
                latency_ms=200.0, judge_invoked=False,
                judge_latency_ms=None, complexity_score=0.5,
                impact_scope="MEDIUM", cost=0.01, total_tokens=100,
            )
...
class TestStateManagerWAL:
    ...
    def temp_config_and_wal(self):
        ...
        from sentinelrouter.schemas.config_models import (
            UnifiedConfig, SystemSettings, ModelConfig, ModelState,
            ModelCapabilities, RoutingConfig, RateLimits, TierLimits,
            PricingInfo, CostInfo, JudgeConfig, RoutingOrderConfig,
        )
            ...
            config = UnifiedConfig(
                ...
                models={
                    "test-model": ModelConfig(
                        ...
                        state=ModelState(
                            current_rpm=0, requests_today=0, tokens_today=0,
                            total_cost_session=0.0, last_updated_ts=None, exhausted_until_ts=None
                        )
                    ),
                },
                judge_config=JudgeConfig(model_order=["test-model"], is_judge_required=False),
                routing_order_config=RoutingOrderConfig(strong_models=[], weak_models=["test-model"])
            )
    ...
    async def test_wal_replay_recovers_state(self, temp_config_and_wal):
        ...
        from sentinelrouter.sentinelrouter.state_manager import WriteAheadLog, StateManager
...
class TestLambdaCaptureFix:
    ...
    def test_partial_captures_correct_value(self):
        ...
        assert results == ["client_for_model_a", "client_for_model_b", "client_for_model_c"]
