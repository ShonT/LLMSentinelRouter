...
from sentinelrouter.sentinelrouter.semantic.local_vector_processor import (
    LocalVectorProcessor,
)
from sentinelrouter.sentinelrouter.semantic.api_vector_processor import (
    APIVectorProcessor,
)
from sentinelrouter.sentinelrouter.semantic.factory import (
...
class TestLocalVectorProcessor:
    ...
    @patch(
        "sentinelrouter.sentinelrouter.semantic.local_vector_processor.AutoTokenizer"
    )
    async def test_get_embedding_with_mock(
        self, mock_tokenizer_class, mock_onnx, processor
    ):
...
class TestAPIVectorProcessor:
    ...
    def processor(self):
        return APIVectorProcessor(
            api_key="test-key", provider="openai", similarity_threshold=0.85
        )
    ...
    async def test_openai_api_call_success(self):
        ...
        processor = APIVectorProcessor(
            api_key="test-key", provider="openai", model="text-embedding-3-small"
        )
        ...
        with patch.object(
            processor.client, "post", new_callable=AsyncMock
        ) as mock_post:
    ...
    async def test_voyage_api_call_format(self):
        ...
        processor = APIVectorProcessor(
            api_key="voyage-key", provider="voyage", model="voyage-2"
        )
    ...
    async def test_api_error_handling_401(self):
        ...
        processor = APIVectorProcessor(api_key="invalid-key", provider="openai")
    ...
    async def test_api_error_handling_rate_limit(self):
        ...
        processor = APIVectorProcessor(api_key="test-key", provider="openai")
    ...
    async def test_unsupported_provider_initialization(self):
        ...
        with pytest.raises(ValueError, match="Unsupported provider"):
            APIVectorProcessor(api_key="test-key", provider="unsupported-provider")
...
class TestSemanticProcessorFactory:
    ...
    def test_create_simhash_processor(self):
        ...
        processor = SemanticProcessorFactory.create(
            strategy=SemanticStrategy.SIMHASH, similarity_threshold=0.85
        )
    ...
    def test_create_local_vector_processor(self):
        ...
        processor = SemanticProcessorFactory.create(
            strategy=SemanticStrategy.VECTORDB_LOCAL, similarity_threshold=0.90
        )
    ...
    def test_create_api_processor_without_key_raises(self):
        ...
        with pytest.raises(ValueError, match="api_key required"):
            SemanticProcessorFactory.create(strategy=SemanticStrategy.VECTORDB_API)

    def test_create_invalid_strategy_raises(self):
        """Test that invalid strategy raises error."""
        with pytest.raises(ValueError):
            SemanticProcessorFactory.create(strategy="INVALID_STRATEGY")  # type: ignore
...
class TestVectorStore:
    ...
    async def test_add_vector_embedding(self):
        ...
        try:
            ...
            with patch(
                "sentinelrouter.sentinelrouter.semantic.vector_store.lancedb",
                mock_lancedb,
            ):
                with patch(
                    "sentinelrouter.sentinelrouter.semantic.vector_store.pyarrow",
                    mock_pa,
                ):
                    store = VectorStore(use_lancedb=True)
    ...
    async def test_search_simhash_not_implemented(self):
        ...
        results = await store.search_similar(
            embedding=12345678901234567890, top_k=5  # int = SimHash
        )
...
class TestSemanticStrategies:
    ...
    async def test_memory_footprint_comparison(self):
        ...
        api_proc = SemanticProcessorFactory.create(
            SemanticStrategy.VECTORDB_API, api_key="test-key"
        )
