...
class MockLLMClient(BaseLLMClient):
    ...
    def __init__(self, provider_id: str, should_fail: bool = False, fail_count: int = 0):
...
async def test_circuit_breaker_recovery():
    ...
    health_tracker = ProviderHealthTracker(
        failure_threshold=2,
        cooldown_seconds=2  # Very short for testing
    )
    registry = ModelRegistry(health_tracker=health_tracker)
    ...
    for i in range(2):
        await registry.call_with_failover(ModelTier.WEAK, messages=[{"role": "user", "content": "test"}])
...
async def test_three_tier_failover():
    ...
    for provider_id, priority, should_fail in models:
        ...
        print(f"   Registered: {provider_id} (priority={priority}, fails={should_fail})")
