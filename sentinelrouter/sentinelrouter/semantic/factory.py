...
class SemanticProcessorFactory:
        ...
        **kwargs,
        ...
        if strategy == SemanticStrategy.SIMHASH:
            ...
            return SimHashProcessor(
                hash_bits=hash_bits, hamming_threshold=hamming_threshold
            )

        elif strategy == SemanticStrategy.VECTORDB_LOCAL:
            # Mode B: Local vectors with ONNX (default)
            return LocalVectorProcessor(
                model_path=model_path, similarity_threshold=similarity_threshold
            )
    ...
    def from_settings(settings) -> SemanticProcessor:
        ...
        try:
            ...
            logger.warning(
                f"Invalid semantic strategy '{strategy_str}', defaulting to VECTORDB_LOCAL"
            )
        ...
        if strategy == SemanticStrategy.SIMHASH:
            kwargs["hamming_threshold"] = getattr(
                settings, "cycle_detection_simhash_threshold", 3
            )
            ...
            api_key = getattr(settings, "semantic_api_key", None) or getattr(
                settings, "openai_api_key", None
            )

            if not api_key:
                logger.warning(
                    "No API key found for VECTORDB_API, falling back to VECTORDB_LOCAL"
                )
                strategy = SemanticStrategy.VECTORDB_LOCAL
            else:
                kwargs["api_key"] = api_key
                kwargs["provider"] = getattr(
                    settings, "semantic_api_provider", "openai"
                )
                kwargs["model"] = getattr(
                    settings, "semantic_api_model", "text-embedding-3-small"
                )

        return SemanticProcessorFactory.create(
            strategy=strategy, similarity_threshold=similarity_threshold, **kwargs
        )
