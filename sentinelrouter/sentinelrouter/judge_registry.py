...
class JudgeRegistry:
    ...
    def __init__(self, health_tracker: Optional[JudgeHealthTracker] = None):
        ...
        self._default_fallback = (
            0.1,
            "LOW",
            "All judges failed, assuming weak model (low complexity).",
        )
    ...
    def register_judge(self, judge: JudgeModel) -> None:
        ...
        logger.info(f"Registered judge: {judge.judge_id} (priority {judge.priority})")
    ...
    def select_judge(self, exclude: Optional[List[str]] = None) -> Optional[JudgeModel]:
        """
    ...
    async def judge_with_failover(
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                if attempt > 0:
                    primary_judge_id = (
                        self._judges[0].judge_id if self._judges else "unknown"
                    )
                ...
                last_error = e
                logger.error(
                    f"❌ Judge {judge.judge_id} failed: {e}. " f"Trying next backup..."
                )
