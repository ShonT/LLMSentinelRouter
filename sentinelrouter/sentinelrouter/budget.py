...
class BudgetKillSwitch:
    ...
    def get_or_create_session(
        ...
        if not session:
            ...
            self.db.commit()
            logger.info(
                f"Created new session {session_id} with tier={session.tier}, budget={session.max_cost_per_session}"
            )
        return session
    ...
    def check_and_reserve_budget(
        self, session_id: str, estimated_cost: float, max_retries: int = 3
        ...
        logger.error(
            f"Failed to reserve budget after {max_retries} attempts for session {session_id}"
        )
        return False, None
    ...
    def release_reserved_budget(
        ...
        if cost_difference > 0 and new_cost > session.max_cost_per_session:
            ...
            new_cost = min(
                new_cost, session.max_cost_per_session * 1.1
            )  # Allow 10% overage
    ...
    def add_cost(self, session_id: str, cost: float) -> None:
        ...
        self.db.commit()
        logger.debug(
            f"Added cost {cost} to session {session_id}. New total: {session.current_cost}"
        )
    ...
    def reset_session(
        self, session_id: str, new_max_cost: Optional[float] = None
    ) -> None:
        """
        ...
        self.db.commit()
        logger.info(
            f"Reset session {session_id} (cost=0, max={session.max_cost_per_session})"
        )
