...
class KeyInstancePool:
    ...
    def order_instances(
        self, instances: List[KeyInstanceRecord]
    ) -> List[KeyInstanceRecord]:
        """Return enabled, available instances ordered by priority."""
        available = [
            instance
            for instance in instances
            if instance.enabled
            and self.health_tracker.is_available(instance.instance_id)
        ]
...
class ModelRegistry:
    ...
    def get_available_providers(
        self, tier: ModelTier, exclude: Optional[List[str]] = None
    ) -> List[ModelProvider]:
        """
    ...
    async def call_with_failover(
        ...
        for attempt in range(max_attempts):
            ...
            try:
                ...
                last_error = e
                logger.error(
                    f"❌ {provider.provider_id} failed: {e}. " f"Trying next backup..."
                )
