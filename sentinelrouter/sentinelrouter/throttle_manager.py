...
class ModelThrottleManager:
    ...
    def __init__(
        ...
        self._ban_history: Dict[
            str, List[float]
        ] = {}  # model_id -> list of ban timestamps
        self._lock = Lock()
    ...
    def record_throttle(self, model_id: str, error_message: str) -> Optional[float]:
        ...
        with self._lock:
            ...
            event = ThrottleEvent(
                timestamp=now, model_id=model_id, error_message=error_message
            )
            ...
            self._throttle_events[model_id] = [
                e for e in self._throttle_events[model_id] if e.timestamp > cutoff
            ]
            ...
            if recent_count >= self.throttle_threshold:
                ban_duration = self._apply_ban(
                    model_id, self._throttle_events[model_id]
                )
                logger.warning(
    ...
    def _apply_ban(self, model_id: str, throttle_events: List[ThrottleEvent]) -> float:
        ...
        self._ban_history[model_id] = [
            t for t in self._ban_history[model_id] if t > extended_cutoff
        ]
    ...
    def get_status(self) -> Dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "total_throttle_events": sum(
                    len(events) for events in self._throttle_events.values()
                ),
