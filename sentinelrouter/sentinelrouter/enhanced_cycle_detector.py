...
class EnhancedCycleDetector:
    ...
    def __init__(
        ...
        if enable_storage:
            self.vector_store = VectorStore(
                storage_path=f"data/vectors/{session_id}", use_lancedb=True
            )

        # Graph for cycle detection (optional, if networkx available)
        if nx is None:
            logger.warning(
                "networkx not installed. Graph visualization will be disabled."
            )
            self.graph = None
        else:
        ...
        # In-memory tracking
        self.recent_prompts: List[
            Tuple[object, datetime]
        ] = []  # (embedding, timestamp)
        self.recent_interactions: List[
            Tuple[object, datetime]
        ] = []  # (prompt+response embedding, timestamp)
    ...
    async def detect_cycle_with_prompt(self, prompt: str) -> bool:
        ...
        recent_prompts = [
            (emb, ts)
            for emb, ts in self.recent_prompts[-recent_prompt_limit:]
        ...
        for existing_embedding, ts in recent_prompts:
            similarity = await self.semantic_processor.similarity(
                prompt_embedding, existing_embedding
            )
    ...
    async def _detect_cycle(self, embedding: object) -> bool:
        ...
        for existing_embedding, ts in self.recent_interactions[
            -10:
    ...
    async def is_similar_to_recent(
    ...
    def get_memory_usage_mb(self) -> float:
        ...
        storage_mem = (
            self.vector_store.get_memory_usage_mb() if self.vector_store else 0.0
        )
