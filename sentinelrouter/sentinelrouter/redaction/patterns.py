...
class RedactionPattern(NamedTuple):
    ...
    regex: re.Pattern
    description: str
    group_name: str = None  # Optional: specific group to redact (e.g., password in connection string)
...
CLOUDS_AND_PII_PATTERNS: List[RedactionPattern] = [
    ...
    RedactionPattern(
        "AWS Session Token",
        re.compile(r"(?i)aws(.{0,20})?session(.{0,20})?token(.{0,20})?['\"][A-Za-z0-9/+=]{100,500}['\"]"),
    ...
    RedactionPattern(
        ...
        re.compile(r'"type":\s*"service_account"[^}]{100,1000}"private_key":\s*"[^"]{1,2000}"'),
    ...
    RedactionPattern(
        "Azure Client Secret",
        re.compile(r"(?i)azure(.{0,20})?client.?secret(.{0,20})?['\"\s][a-zA-Z0-9.~_-]{32,}['\"]"),
        "Matches Azure AD App client secrets"
    ),
    RedactionPattern(
        "Azure Subscription Key",
        re.compile(r"(?i)(?:ocp-apim-subscription-key|api-key)(.{0,10})?['\"\s][a-f0-9]{32}['\"]"),
    ...
    RedactionPattern(
        "US Phone Number",
        re.compile(r"\b(?:\+?1[-.\s]?)?\(?([2-9][0-9]{2})\)?[-.\s]?([0-9]{3})[-.\s]?([0-9]{4})\b"),
    ...
    RedactionPattern(
        "MySQL Connection",
        re.compile(r"mysql://(?P<user>[^:]+):(?P<pass>[^@]+)@(?P<host>[^/:]+)(?::(?P<port>\d+))?/(?P<db>[^\s?]+)"),
        "Matches MySQL connection strings",
        group_name="pass"
    ),
    RedactionPattern(
        "MongoDB Connection",
        re.compile(r"mongodb(?:\+srv)?://(?P<user>[^:]+):(?P<pass>[^@]+)@(?P<host>[^/?]+)"),
    ...
    RedactionPattern(
        "Generic API Key",
        re.compile(r"(?i)(?:api[_-]?key|apikey)(.{0,10})?['\"\s]([a-zA-Z0-9_\-]{20,})['\"]"),
...
PATTERN_CATEGORIES = {
    ...
    "PII": ["US SSN", "Credit Card", "US Phone Number", "Email Address", "IPv4 Address"],
