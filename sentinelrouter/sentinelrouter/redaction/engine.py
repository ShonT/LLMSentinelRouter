...
class RedactionResult:
    ...
    def __init__(
        ...
        ],  # (pattern_name, matched_value, start, end)
    ...
    def scrub(self, text: str) -> RedactionResult:
        ...
        for pattern in self.patterns:
            for match in pattern.regex.finditer(text):
                ...
                patterns_triggered[pattern.name] = (
                    patterns_triggered.get(pattern.name, 0) + 1
                )
    ...
    def scrub_dict(self, data: Dict) -> Dict:
        ...
        for key, value in data.items():
            if isinstance(value, str):
                ...
                result[key] = [
                    self.scrub(item).redacted_text
                    if isinstance(item, str)
                    else self.scrub_dict(item)
