...
class MetricsCollector:
    ...
    def __init__(self, storage_dir: str = "./data/metrics", max_size_mb: int = 200):
        ...
        logger.info(
            f"MetricsCollector initialized: {self.storage_dir}, max_size={max_size_mb}MB, loaded {len(self.recent_metrics)} recent metrics"
        )
    ...
    def _check_and_rotate_if_needed(self):
        ...
        if file_size >= self.max_size_bytes:
            logger.warning(
                f"Metrics file exceeded {self.max_size_bytes} bytes, rotating..."
            )
    ...
    def record_model_latency(
        self, model_id: str, route_type: str, latency_ms: float, status: str
    ):
    ...
    def record_tokens_per_second(
        self, model_id: str, route_type: str, tps: float, total_tokens: int
    ):
    ...
    def record_semantic_cache_event(
        self,
        event: str,
        semantic_hash: str,
        hit: bool,
    ...
    def record_judge_timeout_escalation(
        self, session_id: str, model_id: str, timeout_ms: float
    ):
    ...
    def record_event(self, event_type: str, data: Dict[str, Any]):
        ...
        """
        metric = {"type": event_type, "timestamp": time.time(), **data}
        self._write_metric(metric)
    ...
    def get_aggregated_stats(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "weak_model_latency": self._aggregate_latency(
                metrics, "weak_model_latency"
            ),
            "strong_model_latency": self._aggregate_latency(
                metrics, "strong_model_latency"
            ),
    ...
    def _aggregate_latency(
        self, metrics: List[Dict], metric_type: str
    ) -> Dict[str, Any]:
