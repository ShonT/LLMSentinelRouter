...
class JudgePolicy(BaseModel):
    ...
    enabled: bool = Field(
        default=False, description="Whether the judge system is active."
    )
...
class SemanticCachePolicy(BaseModel):
    ...
    enabled: bool = Field(
        default=False, description="Whether semantic caching is active."
    )
...
class CycleDetectionPolicy(BaseModel):
    ...
    enabled: bool = Field(
        default=True, description="Whether cycle detection is active."
    )
...
class AdminPolicyConfig(BaseModel):
    ...
    judge: JudgePolicy = Field(
        default_factory=JudgePolicy, description="Judge system policy."
    )
    semantic_cache: SemanticCachePolicy = Field(
        default_factory=SemanticCachePolicy, description="Semantic cache policy."
    )
    cycle_detection: CycleDetectionPolicy = Field(
        default_factory=CycleDetectionPolicy, description="Cycle detection policy."
    )
